    ...
    <div class="conteneur_connexion">
      ...
      <div class="menu_connexion">
        <div class="connexion_inscription">
          ...
          <h3 v-if="!status">
            Vous êtes déjà inscrit ?<span v-on:click="toggleStatus">
              Connectez-vous !</span
            >
          </h3>
        </div>
        <form>
          ...
          <label title="Une majuscule, un chiffre et 8 caractères au minimum">
            Mot de passe :
            <input
              type="password"
          ...
          <p v-if="pwd.isValid == 0" class="erreur_formulaire">
          ...
          <p v-if="lastName.isValid == 0" class="erreur_formulaire">
          ...
          <p v-if="firstName.isValid == 0" class="erreur_formulaire">
        ...
        <p
          ...
          v-if="confirmation_inscription == true && status == false"
        >
          Merci de vous être inscrit(e) ! Vous pouvez vous
          <span v-on:click="toggleStatus">connecter</span>.
        </p>
        <p
          class="probleme_connexion"
          v-if="probleme_connexion == true && status == true"
        >
          Veuillez vérifier votre adresse e-mail et/ou votre mot de passe.<br />
          Si vous n'avez pas de compte vous pouvez vous
          <span v-on:click="toggleStatus">inscrire</span>.
        </p>
        <p class="contact_admin">
          Cet espace est réservé aux employés de Groupomania, profitez de cet
          espace pour échanger et vous détendre !
          <br />
          <br />
          Vous avez des difficultés à vous connecter ? Contactez un
          <a href="mailto:admin@groupomania.fr"> administrateur</a>
        </p>
      </div>
    </div>
  </div>
...
export default {
      ...
      fetch(request)
        ...
        .then((value) => {
          ...
          if (value.token != undefined || value.token === "") {
            ...
            this.confirmation_inscription = true;
          }
        })
        .catch(() => {
          this.probleme_connexion = true;
        });
    },
    toggleStatus() {
  ...
  data() {
    return {
      probleme_connexion: false,
      confirmation_inscription: false,
